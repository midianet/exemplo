 ...
 *         imposto:
 *           type: string
 *           description: Valor do imposto do produto
 *         valor:
 *           type: double
 ...
 */
router.get("/", (req, res) => {
	const produtos = JSON.parse(JSON.stringify(req.app.db.get("produtos").filter({})));
	produtos.forEach(produto => delete produto.imposto);
	res.send(produtos);
});
